...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Для каждого ТабДокумент из РезультатФункции.МассивТабличныхДокументов цикл
		ТабДокумент.Показать(NStr("ru = 'Кто может согласовать'"));
	Конеццикла;	
...
Функция ПолучитьТекЗадачиИИсполнители(БПСсылка)  
	...
	Если БПСсылка.Стартован
		...
		Если РезультатЗапроса.Пустой() Тогда
			МассивСообщений.Добавить(NStr("ru = 'Не удалось получить не выполненные задачи для '") + БПСсылка);
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ТЗИсполнителейЗадачи.Количество() = 0 Тогда
					МассивСообщений.Добавить(NStr("ru = 'Не удалось получить текущую задачу и ее исполнителей.
                                                   |Проверьте правильность заполнения регистра адресации.'"));
				Иначе
	...
	Если НЕ БПСсылка.Стартован Тогда
		МассивСообщений.Добавить(NStr("ru = 'Бизнес процесс еще не стартован, нет текущей задачи и исполнителя'"));
	Конецесли;		
	Если БПСсылка.Завершен Тогда
		МассивСообщений.Добавить(NStr("ru = 'Бизнес процесс уже завершен, нет текущей задачи и исполнителя'"));
	Конецесли;
...
Функция ПолучитьТабДокументДляЗадачи(пЗадача, ТЗИсполнителейЗадачи,БПСсылка) 
	...
	Если пЗадача.ТочкаМаршрута = ПредопределенноеЗначение("BusinessProcess.бпсСогласование.RoutePoint.ОбработатьРезультатыСогласования") Тогда
		ТекстЗаголовка = StrTemplate(NStr("ru = '[%1] возвращен на доработку исполнителю:'"), БПСсылка.ОбъектБД);
	Иначе
		ТекстЗаголовка = StrTemplate(NStr("ru = 'Кто может согласовать: [%1]'"), БПСсылка.ОбъектБД);
	Конецесли;
