...
Процедура КомандаОтправитьТестовоеСообщениеНаСервере(АдресЭлектроннойПочтыКому)
	...
	Если ЗначениеЗаполнено(ИдентификаторСообщения) Тогда
		Сообщить(NStr("ru = 'Тестовое сообщение отправлено'"));
	Иначе
		Сообщить(NStr("ru = 'Ошибка! Тестовое сообщение не отправлено'"));
	Конецесли;
КонецПроцедуры
...
Процедура КомандаОтправитьТестовоеСообщение(Команда)
	...
	ПоказатьВводЗначения(Оповещение,ВыбЗнач, NStr("ru = 'Укажите адрес электронной почты куда отправить тестовое письмо'"), ОписаниеТипов);
