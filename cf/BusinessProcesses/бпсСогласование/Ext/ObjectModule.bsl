...
Процедура ЗаполнитьАвто(Отказ) Экспорт
	Если НЕ ЗначениеЗаполнено(Инициатор) Тогда
		...
		Инициатор = ПараметрыСеанса.CurrentUser;
	Конецесли;
...
Функция СформироватьТекстИТемуУведомленияПоУсловиюОтправки(СобытиеДляОтправкиУведомления) Экспорт 
	...
	Если СобытиеДляОтправкиУведомления = Перечисления.бпсСобытия.ПриЗавершении Тогда
		...
		РезультатСогласования = ?(Согласовано,
										NStr("ru = 'Утверждена'"),
										NStr("ru = 'Отклонена'"));
		ТемаУведомления  = StrTemplate(NStr("ru = 'Уведомление 1С: согласование завершено
                                             |Статус [%1]
                                             |Объект [%2]'"), РезультатСогласования, ОбъектБД);
		
		ТекстУведомления = StrTemplate(NStr("ru = 'Уведомление 1С: согласование завершено
                                             |Статус: [%1]
                                             |Объект: [%2]
                                             |Предмет согласования: [%3]
                                             |Тип: [%4]'"), РезультатСогласования, ОбъектБД, ПредметСогласования, ТипЗнч(ОбъектБД)) + Символы.ПС;
	...
	ИначеЕсли СобытиеДляОтправкиУведомления = Перечисления.бпсСобытия.ПриСтарте
		...
		ТемаУведомления  = StrTemplate(NStr("ru = 'Уведомление 1С: запущено согласование для объекта [%1]'"), ОбъектБД);
		ТекстУведомления = StrTemplate(NStr("ru = 'Уведомление 1С: запущено согласование для
                                             |Объекта: [%1]
                                             |Статус: [%2]
                                             |Предмет согласования: [%3]
                                             |Тип: [%4]'"), ОбъектБД, пСтатус, ПредметСогласования, ТипЗнч(ОбъектБД)) + Символы.ПС;
	...
	ИначеЕсли СобытиеДляОтправкиУведомления = Перечисления.бпсСобытия.ПриВыполненииЗадачиВОчереди Тогда	
		...
		пСтатус = ПредопределенноеЗначение("Справочник.бпсСтатусыОбъектов.Согласование_ВПроцессеСогласования");
		ТемаУведомления  = StrTemplate(NStr("ru = 'Уведомление 1С:
                                             |Выполнена задача шаг № %1 из %2
                                             |Объект [%3]'"), ТекущийНомерОчереди, КоличествоШаговДляСогласования, ОбъектБД);
		
		ТекстРезультатВыполненияЗадачи = NStr("ru = ' с результатом '") + 
											?(РезультатВыполненияЗадачи, NStr("ru = 'СОГЛАСОВАНО'"), NStr("ru = 'НЕ СОГЛАСОВАНО'"));
		
		ТекстУведомления = StrTemplate(NStr("ru = 'Уведомление 1С: 
                                             |Выполнена задача шаг № %1 из %2
                                             |%3
                                             |Объект: [%4]
                                             |Предмет согласования: [%5]
                                             |Тип: [%6]'"), 
								ТекущийНомерОчереди, 
								КоличествоШаговДляСогласования, 
								ТекстРезультатВыполненияЗадачи, 
								ОбъектБД, 
								ПредметСогласования, 
								ТипЗнч(ОбъектБД)) + Символы.ПС;
	Иначе
		ВызватьИсключение NStr("ru = 'Ошибка! Не задан алгоритм формирования уведомления, для условия: '") + СобытиеДляОтправкиУведомления;
		ТемаУведомления = "";
		ТекстУведомления = "";
		Отказ = Истина;
	Конецесли;
...
Функция ПолучитьТекстРецензий() Экспорт
	...
	ТекстРецензий = Символы.ПС + Символы.ПС + NStr("ru = 'РЕЦЕНЗИИ:'") + Символы.ПС + Символы.ПС;
	Выборка = РезультатЗапроса.Выбрать();
	
	Пока Выборка.Следующий() Цикл                                                                 
		ТекстРецензий = ТекстРецензий + StrTemplate(NStr("ru = 'Дата рецензии: %1
                                                          |Рецензент: [%2]
                                                          |Роль: [%3]
                                                          |
                                                          |Рецензия: '"), Выборка.ДатаРецензии, Выборка.Рецензент, Выборка.РольАдресации) + 
							Символы.ПС + " - " + СокрЛП(Выборка.Рецензия) + Символы.ПС + Символы.ПС;
	КонецЦикла;
	Возврат ТекстРецензий;
...
Функция СформироватьТекстПояснения(Отказ) Экспорт 
	ТекстПояснения = StrTemplate(NStr("ru = 'Прошу согласовать:
                                      |ОбъектБД: [%1]
                                      |Предмет согласования: [%2]'"), ОбъектБД, ПредметСогласования);
...
Процедура ЗаполнитьЛистСогласования(Отказ) Экспорт
	...
	Если ПредметСогласования.ОпределениеЛистаСогласования.Количество() > 0 Тогда
		ПротоколФормированияЛистаСогласования = StrTemplate(NStr("ru = 'Протокол формирования листа согласования:
                                                                 |Объект согласования: [%1]
                                                                 |Предмет согласования: [%2]
                                                                 |
                                                                 |'"), ОбъектБД, ПредметСогласования);
	Конецесли;
	...
	Для каждого СтрокаОпределениеЛистаСогласования из ПредметСогласования.ОпределениеЛистаСогласования цикл
		...
		Если НЕ пУсловиеВыполнено Тогда
			ПротоколФормированияЛистаСогласования = ПротоколФормированияЛистаСогласования + 
								StrTemplate(NStr("ru = 'Условие № %1 [%2] не выполнено, поэтому не добавляем рецензента'"), 
										СтрокаОпределениеЛистаСогласования.НомерСтроки, пУсловие) + Символы.ПС;			
			Продолжить;
		Конецесли;		
		
		Если НЕ ЗначениеЗаполнено(пУсловие) Тогда
			пУсловие = NStr("ru = 'Без условия'");
		Конецесли;
		...
		ПротоколФормированияЛистаСогласования = ПротоколФормированияЛистаСогласования + 
						StrTemplate(NStr("ru = 'Условие № %1 [%2] выполнено, поэтому
				                          |- очередь: %3 требуется согласование с ролью [%4]
				                          |подразделение [%5]'"),
								СтрокаОпределениеЛистаСогласования.НомерСтроки, 
								пУсловие, 
								Очередь,
								СтрокаЛистСогласования.РольАдресации, 
								СтрокаЛистСогласования.ПодразделениеАдресации) + Символы.ПС;
...
Процедура СогласоватьСРецензентомПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ВызватьИсключение NStr("ru = 'Ошибка! Не удалось найти строку в ТЧ: ""Лист согласования"" с номером очереди '") + ТекущийНомерОчереди;
	Конецесли;
...
Процедура ВыполнитьДействие(пДействие,Отказ) Экспорт
	...
	Иначе
		ВызватьИсключение StrTemplate(NStr("ru = 'Ошибка!
                                           |Нет алгоритма для того, чтобы выполнить действие [%1]'"), пДействие);
	Конецесли;
...
Функция ПолучитьНаименованиеЗадачи(ТочкаМаршрута = Неопределено) Экспорт 	
	...
	Если ТочкаМаршрута = БизнесПроцессы.бпсСогласование.ТочкиМаршрута.ОбработатьРезультатыСогласования Тогда
		НаименованиеЗадачи = StrTemplate(NStr("ru = 'Необходимо исправить: [%1]'"), ОбъектБД);
	Иначе
		НаименованиеЗадачи = StrTemplate(NStr("ru = Согласуйте: [%1]'"), ОбъектБД);	
	КонецЕсли;
...
Процедура УсловиеСогласоватьПовторноПроверкаУсловия(ТочкаМаршрутаБизнесПроцесса, Результат)
	...
	Если Результат Тогда
		...
		Если Отказ Тогда
			ВызватьИсключение NStr("ru = 'Ошибка! Не удалось выполнить действие'");
		Конецесли;
...
Процедура ВыполнитьЗадачу(ДопПараметры) Экспорт
	...
	Иначе
		ВызватьИсключение StrTemplate(NStr("ru = 'Ошибка! Нет алгоритма выполнения задачи для команды [%1]'"), КомандаСогласования);
	Конецесли;
	...
	Если ЭтоКомандаСогласования_Согласовано 
		...
		Если Отказ Тогда
			ВызватьИсключение NStr("ru = 'Ошибка! не удалось выполнить задачу'");
		Конецесли;
