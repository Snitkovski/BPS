Функция ДобавитьСообщение(ДопПараметры) Экспорт
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ПользовательДляОтправки) Тогда
			ТекстОшибки = NStr("ru = 'Ошибка! Не указан пользователь для отправки уведомления'");
		...
		Если НЕ ЗначениеЗаполнено(НастройкиПользователя) Тогда
			...
			ТекстОшибки = StrTemplate(NStr("ru = 'ВНИМАНИЕ! Не удалось отправить уведомление для пользователя [%1].
                                            |Проверьте настройки пользователя в регистре сведений бпсНастройкиПользователей'"), ПользовательДляОтправки);
		...
		Если НЕ ЗначениеЗаполнено(АдресЭлектроннойПочтыКому) Тогда
			ТекстОшибки = StrTemplate(NStr("ru = 'ВНИМАНИЕ! Не заполнен адрес электронной почты [%1].
                                            |Проверьте настройки пользователя в регистре сведений бпсНастройкиПользователей'"), ПользовательДляОтправки);
			РегистрыСведений.бпсОшибки.ДобавитьОшибку(ТекстОшибки, ПредопределенноеЗначение("Перечисление.бпсТипыОшибок.НеЗаполненыНастройкиПользователя"));					
			Возврат Неопределено;
		Конецесли;
...
Процедура ВыполнитьРассылкуТелеграмм() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Ответ <> Неопределено И Ответ.КодСостояния = 200 Тогда
			СообщениеОтправлено = Истина;
		Иначе	
			ЗаписьЖурналаРегистрации(NStr("ru = 'Телеграмм.Отправка'"),
					УровеньЖурналаРегистрации.Ошибка,,,
					StrTemplate(NStr("ru = 'Не смогли отправить сообщение телеграмм пользователю [%1]'"), Выборка.ИДПользователяТелеграмм));
		КонецЕсли;
		...
		Пока ВыборкабпсСообщения.Следующий() Цикл
			...
			Иначе
				...
				Запись.ИнформацияООшибке = Запись.ИнформацияООшибке + 
											...
											StrTemplate(NStr("ru = ' Не смогли отправить сообщение телеграмм пользователю [%1]'"), 
														Выборка.ИДПользователяТелеграмм);
			Конецесли;
			Запись.Записать();
		Конеццикла;
	КонецЦикла;
