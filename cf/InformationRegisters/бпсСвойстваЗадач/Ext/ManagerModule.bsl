Функция ПолучитьЗначениеСвойстваЗадачи(ЗадачаСсылка,ИмяСвойства) Экспорт 
	...
	Если НЕ ЗначениеЗаполнено(ЗадачаСсылка) Тогда
		ВызватьИсключение NStr("ru = 'Ошибка! Не заполнен параметр ЗадачаСсылка'");
	Конецесли;	
	
	Если НЕ ЗначениеЗаполнено(ИмяСвойства) Тогда
		ВызватьИсключение NStr("ru = 'Ошибка! Не заполнен параметр ИмяСвойства'");
	Конецесли;
	...
	Запрос.УстановитьПараметр("Пользователь", ПараметрыСеанса.CurrentUser);
...
Процедура УстановитьЗначениеСвойстваЗадачи(ЗадачаСсылка,ИмяСвойства,ЗначениеСвойства) Экспорт
	...
	МенеджерЗаписи.Пользователь = ПараметрыСеанса.CurrentUser;
