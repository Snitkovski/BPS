Процедура СформироватьЗаписи(ДопПараметры,Отказ) Экспорт
	Если НЕ ДопПараметры.Свойство("ЗадачаСсылка") Тогда 
		Сообщить(NStr("ru = 'Ошибка! Не все параметры заданы для формирования записей в РС: бпсЗадачиПоБизнесПроцессам'"));
		Отказ = Истина;
		Возврат;
	Конецесли;
...
Функция ПолучитьПараметрыДляЗаписи(ЗадачаСсылка)
	...
	Если ЗадачаСсылка.ТочкаМаршрута = ПредопределенноеЗначение("BusinessProcess.бпсСогласование.RoutePoint.ОбработатьРезультатыСогласования") Тогда
		ВыводитьВОтчет = Ложь;
	КонецЕсли;
