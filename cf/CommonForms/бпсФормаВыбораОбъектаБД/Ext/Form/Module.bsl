...
Процедура ВыборОбъектаБД()
	...
	Иначе
		...
		Сообщение.Текст = NStr("ru = 'Укажите объект согласования и нажмите выбрать'");
		Сообщение.Сообщить();
	Конецесли;		
...
Функция ПолучитьМенеджерОбъектаДляЗапроса(пМенеджерОбъекта) Экспорт 
	...
	Если пМенеджерОбъекта = "Справочники" or пМенеджерОбъекта = "Catalogs" Тогда
		МенеджерОбъектаДляЗапроса = "Справочник";
	//ИначеЕсли пМенеджерОбъекта = "Документы" Тогда
	ИначеЕсли пМенеджерОбъекта = "Документы" or пМенеджерОбъекта = "Documents" Тогда
		МенеджерОбъектаДляЗапроса = "Документ";
	Иначе
		ВызватьИсключение NStr("ru = 'Не удалось получить менеджер объекта для запроса для МенеджерОбъекта '") + пМенеджерОбъекта;
	Конецесли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//  SAM  {
	// сортировка Дерева Значений с использованием объекта сравнения значений
	пДЗОбъектыБД.Строки.Сортировать("ОбъектБДСиноним", True, Новый СравнениеЗначений);
	//  }  SAM
