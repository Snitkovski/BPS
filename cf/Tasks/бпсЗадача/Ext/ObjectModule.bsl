...
Процедура ОтправитьУведомлениеОЗадаче(Отказ) Экспорт
	...
	Если НЕ ДополнительныеСвойства.Свойство("бпсЭтоНовый") ИЛИ ДополнительныеСвойства.бпсЭтоНовый = Ложь Тогда
		Возврат;
	Конецесли;
	...
	Если ТЗИсполнителейЗадачи.Количество() = 0 Тогда
		ТекстОшибки = StrTemplate(NStr("ru = 'ВНИМАНИЕ! Не удалось определить исполнителей задачи для
                                        |РольАдресации [%1]
                                        |ПодразделениеАдресации [%2]'"), РольАдресации, ПодразделениеАдресации);
	...
	Для каждого СтрокаТЗИсполнителейЗадачи из ТЗИсполнителейЗадачи цикл
		...
		ТемаУведомления = NStr("ru = 'В системе 1С Вам поступила задача'");
		ТекстУведомления = NStr("ru = 'Вам поступила задача: '") + Наименование + Символы.ПС;
...
Процедура СформироватьЗаписиРС_бпсЗадачиПоБизнесПроцессам(Отказ) Экспорт	
	...
	Если НЕ бпсЭтоНовый ИЛИ НЕ ЗначениеЗаполнено(бпсЭтоНовый) Тогда
		Возврат;
	Конецесли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если Выполнена Тогда
		...
		ФактическийИсполнитель = ПараметрыСеанса.CurrentUser;		
