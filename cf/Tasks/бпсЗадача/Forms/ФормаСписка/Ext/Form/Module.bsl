...
Процедура ОбработатьВыборЗадачи(ЗадачаСсылка,СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(БПСсылка) = Тип("БизнесПроцессСсылка.бпсСогласование") тогда
    	...
    	ЭтоТочкаМаршрута_ОбработатьРезультатыСогласования = ТочкаМаршрута = ПредопределенноеЗначение("BusinessProcess.бпсСогласование.RoutePoint.ОбработатьРезультатыСогласования");
...
Процедура КомандаПоказатьСкрытыеЗадачи(Команда)
	...
	Если НеПоказыватьСкрытыеЗадачи Тогда
		КнопкаПоказатьСкрытыеЗадачи.Заголовок = NStr("ru = 'Показать скрытые задачи'");
	Иначе
		КнопкаПоказатьСкрытыеЗадачи.Заголовок = NStr("ru = 'Не показывать скрытые задачи'");
	Конецесли;	
КонецПроцедуры
...
Процедура ВыполнитьЗадачуНаСервере(КомандаСогласования) Экспорт
	...
	Для каждого ЗадачаСсылка из МассивЗадач цикл
		Если ЗадачаСсылка.ТочкаМаршрута <> ТочкаМаршрута_СогласоватьСРецензентом Тогда			
			...
			Сообщение.Текст = NStr("ru = 'Ошибка! Кнопки ""Согласовано / Не согласовано"" доступны только для задач согласования'");
			Сообщение.Сообщить();
			Возврат;
		Конецесли;
	Конеццикла;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("Пользователь", ПараметрыСеанса.CurrentUser);
